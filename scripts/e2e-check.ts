/**
 * End-to-end smoke check for SilentPay.
 *
 * Reconnects to the deployed payroll contract, reads its ledger state, and
 * exits 0 on success. Used by `npm run test:e2e` and by the project's CI.
 */
...
import { resolveNetwork, getOrCreateSeed, getDeployment } from '../src/midnight/network';
import { createWallet, persistWalletState } from '../src/midnight/wallet';
...
async function main() {
  ...
  // the contract is indexed and queryable on the chain itself.
